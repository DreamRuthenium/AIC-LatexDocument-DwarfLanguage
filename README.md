# AIC-LatexDocument-DwarfLanguage
An Overleaf+Latex project file which displays the interpretation result of an artificial Language in the game named "Alice in Cradle".

Download pdf files [here](https://github.com/DreamRuthenium/AIC-LatexDocument-DwarfLanguage/releases/tag/PDF-Release).

We used [bitsnpicas](https://github.com/kreativekorp/bitsnpicas), which is a very useful open-source project, to draw all dwarf language symnbols.

File structure:
```
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% File structure
%
% Root
%root/
%├── Figs/                            All figures
%│
%├── Alice.ttf                        ttf file for dwarf language
%│
%├── Alice.kbitx                      design file for dwarf language
%│
%├── BitsNPicas.jar                   Program to edit Alice.kbits(You can also find it on github)
%│
%├── Chinese.tex                      Chinese tex file
%│
%├── HYPixel11pxU-2.ttf               Outside ttf file
%│
%├── Japanese.tex                     Japanese tex file
%│
%├── logo_type_gothic.otf             Outside ttf file
%│
%├── UtsukushiFONT.otf                Outside ttf file
%│
%└── x12y16pxMaruMonica.ttf           Outside ttf file
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
```
